...
begin
  ...
  Begin
      ...
      if Length(DBGrid1.DataSource.DataSet.Fields.FieldByName('user_label').AsString) > 20 then
        ...
        begin
          UserLabel1 := Copy(DBGrid1.DataSource.DataSet.Fields.FieldByName('user_label').AsString, 0, 20); //Copy: Beschr‰nken auf 20 Zeichen
          UserLabel2 := ' ';
        end;
    ...
    // ####################################
      ...
      // select the printer first (either selected already or dialog will appear)
      // this procedures sets the MyPrinterName Variable
      printerSelected := SelectPrinter;      // false if no printer selected

      // Print if printer was actually selected and noc canceled
      if printerSelected = True then
        begin
          PrintLabelVar(MyPrinterName, SampleIDString, UserLabel1, UserLabel2, //Printer, L1, L2
            UserNew, 'Q000' + Count, SampleNumber, L1L1, L1L2, L1L3, L1L4, L1L5, L1bc,
